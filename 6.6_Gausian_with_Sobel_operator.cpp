...
int main(int argc, char* argv[]) {
	...
	Mat sob = Gaus.clone();
	int Skernel = 3;
	int elements = Skernel * Skernel;
	int Gx = 0;
	int Gy = 0;
	int G = 0;
	int T = 35;

	for (int i = Skernel / 2; i < h - (Skernel / 2); i++) {
		for (int j = Skernel / 2; j < w - (Skernel / 2); j++) {
			Gx = (Gaus.at<uchar>(i - 1, j - 1) +
				2 * Gaus.at<uchar>(i, j - 1) +
				Gaus.at<uchar>(i + 1, j - 1) -
				Gaus.at<uchar>(i - 1, j + 1) -
				2 * Gaus.at<uchar>(i, j + 1) -
				Gaus.at<uchar>(i + 1, j + 1));

			Gy = (Gaus.at<uchar>(i - 1, j - 1) +
				2 * Gaus.at<uchar>(i - 1, j) +
				Gaus.at<uchar>(i - 1, j + 1) -
				Gaus.at<uchar>(i + 1, j - 1) -
				2 * Gaus.at<uchar>(i + 1, j) -
				Gaus.at<uchar>(i + 1, j + 1));
	...
	imshow("Gausssian Smoothed and Sobel ", sob);
